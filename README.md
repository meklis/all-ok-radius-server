...
```
ip;client_mac                      # без пристрою і порту, наприклад "smart"
ip;client_mac;device_mac;port      # повна прив'язка, наприклад "clients"
```

Потрібно **рівно 2 або рівно 4+ поля** - рядок з 3 полями не є окремим форматом:
`device_mac`/`port` з нього просто не будуть розібрані (запис поводиться як 2-польний).
